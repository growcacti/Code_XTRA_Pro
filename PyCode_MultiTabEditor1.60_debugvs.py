...
from tkinter import filedialog, messagebox, font, Toplevel, Scrollbar
...
class MultiTabTextEditor:
    def __init__(self, root):
        ...
        self.directory_name = os.path.dirname(self.full_path)
    ...
    def openfile(self):
        ...
        if self.file_path:
            ...
            self.notebook.tab(self.notebook.select(), text=self.file_path.split("/")[-1])
            self.add_to_history(self.file_path)
    def save_file_as(self, event=None):
        ...
        if input_file_name:
            ...
            self.root.title(f"{os.path.basename(self.file_name)} - {self.program_name}")
    ...
    def savefile(self):
        ...
        self.file_path = filedialog.asksaveasfile(filetypes=[("Python Files", "*.py"),("Text Files", "*.txt"), ("All Files", "*.*")])
        if self.file_path:
    ...
    def add_to_history(self, file_path):
        if self.file_path in self.file_history:
            self.file_history.remove(self.file_path)
        self.file_history.insert(0, self.file_path)
    ...
    def update_recent_menu(self):
        ...
        for self.file_path in self.file_history:
            self.recent_menu.add_command(
                label=self.file_path,
                command=lambda path=self.file_path: self.open_recent_file(path),
            )
    ...
    def open_recent_file(self, file_path):
        if os.path.exists(self.file_path):
            ...
            print(f"Opened recent file: {self.file_path}")
        else:
    ...
    def readlines(self, event=None):
        input_file_name = filedialog.askopenself.filename(
    ...
    def save_incremented_version(self):
        if not self.file_name:
            self.full_path = os.path.abspath(__file__)
            self.directory_= os.path.dirname(self.full_path)
            self.filename = os.path.basename(self.full_path)
    ...
    def remove_text_between_quotes(self, quote_type="all"):
        try:
            ...
            patterns = {
                "single": r"'[^']*'",
                "double": r'"[^"]*"',
                "triple": r'"""[\s\S]*? self.get_current_text_widget()"""',
            }
    ...
    def at_cursor(self, textwidget):
        ...
        self.textwidget = textwidget
        btn = tk.Button(top, bd=3, bg="light green", command=lambda : self.about.submit(self.entry,self.textwidget))
        btn.grid(row=3, column=1)
        return self.entry, self.textwidget

    def submit(self,entry,textwidget):
        word = self.entry.get()
    ...
    def open_file_browser(self):
        ...
        self.top.attributes("-topmost", True)  # Make the window topmost
        self.frm = tk.Frame(self.top, bg="thistle2", bd=15)
        ...
        self.lb = tk.Listbox(self.frm, bg="seashell2", height=20, width=60, exportselection=True, yscrollcommand=self.scrollbar.set)
    ...
    def on_file_select(self, event):
        ...
        if selection:
            self.filename = self.lb.get(selection[0])
            self.display_text(self.filename)
    ...
    def recursive_new_list(self):
        ...
        if self.path:
            self.dirpath.delete(0, tk.END)
            self.dirpath.insert(tk.INSERT, self.path)
            self.lb.delete(0, tk.END)
    ...
    def display_text(self, filename, event=None):
        ...
        try:
            with open(filename, "r", encoding="utf-8", errors="ignore") as file:
                content = file.read()
                self.get_current_textwidget().delete("1.0", tk.END)
                self.get_current_textwidget().insert(tk.END, content)
        except Exception as e:
            self.get_current_textwidget().delete("1.0", tk.END)
            self.get_current_textwidget().insert(tk.END, f"Error: {str(e)}")
...
class SyntaxHighlite:
    def __init__(self, textwidget):
        ...
        self.syntax_elements = {
            ...
            "number": r"\b\d+(\.\d+)?\b",
            "string": r".? |\".*?\"",
            "docstring" : r"\b(\"\"\"[\s\S]*?\"\"\"|\"\"\"[\s\S]*?\"\"\")\b",
            'docstring2': r'\b (\'\'\'[\s\S]*?\'\'\'|\'\'[\s\S]*?\'\'\')\b;',
    ...
    def configure_tags(self):
        ...
        self.textwidget.tag_config("classes", foreground="Plum")
        self.textwidget.tag_config("docstring", background="light green")
